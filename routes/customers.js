...
router.get('/:id', async (req, res)=>{
    ...
    if(!customer) res.status(404).send('The customer with the specified id not found');
    res.send(customer);
});
...
router.put('/:id', async (req,res)=>{
    ...
    if(!customer) return res.status(404).send('The customer with the specified id not found');
